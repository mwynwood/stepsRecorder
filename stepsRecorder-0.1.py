...
exported to a self-contained HTML report.
...
Usage:
    ...
    A styled HTML report opens automatically in your browser.
"""
...
SCREENSHOT_DELAY   = 0.10   # seconds after click (let UI react)
OUTPUT_DIR         = "."
# ─────────────────────────────────────────────────────────────────────────────
...
def take_screenshot(x: int, y: int) -> str:
    """Capture full screen, draw highlight at click position, return base64 PNG."""
    ...
    # White crosshair
    lw = 2
    draw.line([x-r+5, y, x+r-5, y], fill=(255, 255, 255, 220), width=lw)
    draw.line([x, y-r+5, x, y+r-5], fill=(255, 255, 255, 220), width=lw)
...
def build_html(steps, title):
    ...
    for s in steps:
        if s["image_b64"]:
            img_html = f'<img src="data:image/png;base64,{s["image_b64"]}" alt="Step {s["num"]}" loading="lazy">'
        else:
            img_html = '<div class="no-img">Screenshot unavailable</div>'
...
.hdr-inner{{max-width:1100px;margin:0 auto;display:flex;align-items:center;justify-content:space-between;gap:20px}}
...
def main():
    ...
    print("  • Click position highlighted with a red marker")
    print("  • Output: self-contained HTML report")
    print()
